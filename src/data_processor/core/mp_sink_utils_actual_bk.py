...
async def send_df_to_mp_old(mp_conf, source_df_xform, data_plan_event, func_args, payload_conf):
    """
        ...
        # async with RetryClient(connector=connector) as session:
            ...
            for batch_source_df in source_df_xform.to_pandas_batches():
                ...
                for rec in batch_source_df.itertuples(index=False):
                    ...
                    payload_list.append(await build_pay_load(
                    ...
                    if len(payload_list) == 100:
                            ...
                            # coroutines.append(send_data(payload_list, payload_conf, session))
                            coroutines.append(asyncio.create_task(send_data(payload_list, payload_conf, session, mp_conf)))
                            payload_list = []
                    else:
                        pass
            ...
            if len(payload_list) > 0:
                coroutines.append(send_data(payload_list, payload_conf, session, mp_conf))
                print('final batch')
                # Execute the coroutines concurrently and handle exceptions
                results = await asyncio.gather(*coroutines, return_exceptions=True)
...
async def send_df_to_mp(
    ...
    # Initialize record count
    record_count = 0
    BATCH_SIZE = mp_conf.get("full_load_params", {}).get(
    ...
    payload_batch_size = mp_conf.get("full_load_params", {}).get(
        "async_payload_batch_size", MP_DEFAULT_VALUES["ASYNC_PAYLOAD_BATCH_SIZE"]
    )
    sleep_time = mp_conf.get("full_load_params", {}).get("async_sleep_time", 3)
...
async def send_data_bk(data, payload_conf, session, mp_conf):
    """
...
async def send_data(data, payload_conf, session, mp_conf):
    ...
    async def add_failed_record(error_status, error_reason):
        failed_records.put(
            (
                datetime.now().strftime("%Y-%m-%d"),
                datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
    ...
    while num_retries <= max_retries:
        try:
            ...
            if num_retries == max_retries + 1:
                await add_failed_record(999, "ServerDisconnectedError")
            else:
                sf_logger.warning(f"Retry attempt {num_retries} of {max_retries}")
            ...
            if num_retries == max_retries + 1:
                await add_failed_record(999, "ClientOSError")
            else:
                sf_logger.warning(f"Retry attempt {num_retries} of {max_retries}")
            ...
            if num_retries == max_retries + 1:
                await add_failed_record(999, "TimeoutError")
            else:
                sf_logger.warning(f"Retry attempt {num_retries} of {max_retries}")
...
async def build_pay_load(
    ...
    if not restart_mp_sink_flag:
        ...
        app_event_attributes.update({'custom_event_type': 'other'})
        print(app_event_attributes)
        mbatch = {
            "events": [{"data": app_event_attributes, 'event_type': 'custom_event'}],
            "source_request_id": str(uuid4()),
            "environment": environment,
            "user_attributes": user_attributes,
            "user_identities": identities,
