...
failed_records = multiprocessing.Queue()    # Create a queue for failed records
count_queue = multiprocessing.Queue()       # Create a queue to store the counts
lock = multiprocessing.Lock()               # Create a lock object

# Create a counter variable to track the size of the queue
failed_records_counter = multiprocessing.Value('i', 0)


def send_df_to_mp(mp_conf, source_df_xform, data_plan_event, func_args, payload_conf, sf_s):
    """
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        try:
            ...
            for batch_source_df in source_df_xform.to_pandas_batches():
                ...
                batch_source_df = batch_source_df.replace({pd.NaT: None}).to_records(index=False)

                if initial_flag:
                    batch_source_df_cum = np.recarray(0, dtype=batch_source_df.dtype)
                    initial_flag = False

                print(type(batch_source_df))
                ...
                if batch_source_df_size >= mp_batch_size:
                    ...
                    for rec in batch_source_df_cum:
                        record_count += 1
                        # print(record_count)
                    ...
                    batch_source_df_size = 0
                    batch_source_df_cum = np.recarray(0, dtype=batch_source_df.dtype)
                    batch_source_df_cum = np.concatenate((batch_source_df_cum, batch_source_df))
                else:
            ...
            for future in concurrent.futures.as_completed(futures):
                if future.exception() is not None:
                    ...
                    failed_rec_count = get_total_failed_rec_count(count_queue)
...
def flush_failed_records(
    ss: Session,
    sf_table_name: str,
    failed_records: multiprocessing.Queue,
...
def get_total_failed_rec_count(count_queue: multiprocessing.Queue):
    """
...
def send_data(data, payload_conf, callback=None):
    ...
    def add_failed_record(error_status, error_reason=None):
        global failed_records_counter
        with failed_records_counter.get_lock():
            failed_records_counter.value += 1
        ...
        if failed_records_counter.value == batch_size:
            flush_failed_records(
                ss,
    ...
    mp_conf = payload_conf.get("mp_conf")
    ...
    while num_retries <= max_retries:
        try:
            response = mparticle.EventsApi(create_mp_configuration(mp_conf)).upload_events(data, callback=callback)
