...
def send_df_to_mp(mp_conf, source_df_xform, data_plan_event, func_args, payload_conf):
    ...
    cumulative_size = 0
    mp_batch_size = 10
    batch_source_df_size = 0
    batch_source_df_cum_size = 0
    numpy_cum_batch_size = 0
    initial_flag = True
    with concurrent.futures.ThreadPoolExecutor(
        ...
        futures = []
        try:
            # print(len(source_df_xform.to_pandas()))
            for batch_source_df in source_df_xform.to_pandas_batches():
                # Calculate the current batch size
                current_batch_size = len(batch_source_df)
                # Update the cumulative batch size
                cumulative_size += current_batch_size
                # Print both current batch size and cumulative batch size
                sf_logger.info(f"Current batch size: {current_batch_size}")
                sf_logger.info(f"Cumulative batch size: {cumulative_size}")

                batch_source_df = batch_source_df.replace({pd.NaT: None})
                print(batch_source_df.dtypes)
                # print(batch_source_df.iloc[0].to_dict())
                # sys.exit(0)
                if initial_flag:
                    print("inside initial record")
                    record_count += 1
                    payload = build_pay_load(
                        mp_conf.get("environment"),
                        data_plan_event,
                        func_args["column_mapping"],
                        next(batch_source_df.itertuples(index=False)),
                        # namedtuple('payload', batch_source_df.columns)(*batch_source_df.iloc[0]),
                        # batch_source_df.iloc[0].to_dict(),
                        mp_conf["restart_from_mp_sink"],
                    )
                    batch_source_df.drop(batch_source_df.index[0], inplace=True)
                    print(len(batch_source_df))
                    send_data(payload, payload_conf)
                    print("finishing initial record")

                    batch_source_df_cum = pd.DataFrame(columns=batch_source_df.columns)
                    initial_flag = False

                # print(len(batch_source_df))
                if batch_source_df_size >= mp_batch_size:
                    # print("I am in if")
                    # print(len(batch_source_df_cum))
                    # for _, rec in batch_source_df.iterrows():
                    numpy_cum_batch_size += len(batch_source_df_cum)
                    print(f"cummulative records {numpy_cum_batch_size}")
                    for rec in batch_source_df_cum.itertuples(index=False):
                        record_count += 1
                        # print(record_count)
                        payload = build_pay_load(
                            mp_conf.get("environment"),
                        ...
                        if record_count == 1:  # first record to handle auth exception
                            ...
                            futures.append(
                                executor.submit(send_data, payload, payload_conf)
                            )
                    print("I am here to initialize")
                    batch_source_df_size = 0
                    batch_source_df_cum = pd.DataFrame(columns=batch_source_df.columns)
                    print(batch_source_df_cum)
                    batch_source_df_cum = pd.concat([batch_source_df_cum, batch_source_df], axis=0)
                    print(batch_source_df_cum)
                else:
                    print(f"numpy size before {len(batch_source_df_cum)}")
                    batch_source_df_size += len(batch_source_df)
                    # print(f"batch size : {batch_source_df_size}")
                    print(batch_source_df_cum)
                    batch_source_df_cum = pd.concat([batch_source_df_cum, batch_source_df], axis=0)
                    print(batch_source_df_cum.dtypes)
                    print(f"numpy size after {len(batch_source_df_cum)}")

            if len(batch_source_df_cum) > 0:
                numpy_cum_batch_size += len(batch_source_df_cum)
                print(f"cummulative records {numpy_cum_batch_size}")
                for rec in batch_source_df_cum.itertuples(index=False):
                    record_count += 1
...
def send_data(data, payload_conf, callback=None):
    ...
    while num_retries <= max_retries:
        try:
            ...
            add_failed_record(400, "Max Retry Error")
            num_retries = max_retries + 1
...
def build_pay_load(
    ...
    print(record)
    print(type(record))
    print(type(record.TIMESTAMP_UNIXTIME_MS_APP_EVENT_ATTRIBUTES))
            ...
            for source_fields, target_field in column_mapping[
                ...
                if column_mapping.get("datatype_mapping"):
                    if (
                        ...
                        == "list"
                    ) and getattr(record, str(source_fields).upper()) is not None:
                        user_attributes[target_field] = json.loads(
                            getattr(record, str(source_fields).upper())
                        )
                    else:
                        user_attributes[target_field] = getattr(record, str(source_fields).upper())
                else:
                    user_attributes[target_field] = getattr(record, str(source_fields).upper())
            mbatch.user_attributes = user_attributes
            sf_logger.debug(f"User attributes built: {mbatch.user_attributes}")

        # data_plan_event_test = data_plan_event.get("event_config")
        #
        # print(data_plan_event_test)
        # # data_plan_event_test.update({'timestamp_unixtime_ms': 1425744000000, "location": 'GeoLocation(latitude=0.00001, latitude = 0.10022}'})
        # # data_plan_event_test.update({'timestamp_unixtime_ms': 1687996800000})
        # print(type(record.TIMESTAMP_UNIXTIME_MS_APP_EVENT_ATTRIBUTES))
        # data_plan_event_test.update({'timestamp_unixtime_ms': record.TIMESTAMP_UNIXTIME_MS_APP_EVENT_ATTRIBUTES})
        #
        # # app_event = mparticle.AppEvent(**data_plan_event.get("event_config"))
        # print(data_plan_event_test)
        # app_event = mparticle.AppEvent(**data_plan_event_test)
        ...
        if "app_event_attributes" in column_mapping:
            for source_fields, target_field in column_mapping[
                ...
                if column_mapping.get("datatype_mapping"):
                    ...
                    app_event_attributes[target_field] = getattr(record, source_fields.upper())
            sf_logger.debug(f"App event attributes built: {app_event_attributes}")

        print(app_event_attributes)
        app_event = mparticle.AppEvent(**app_event_attributes)
        mbatch.events = [app_event]
    else:
