    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        try:
            for batch_source_df in source_df_xform.to_pandas_batches():
                ...
                # test_data = namedtuple('payload', batch_source_df.columns)(*batch_source_df.iloc[0])
                test_data = next(batch_source_df.itertuples(index=False))
                print(test_data)
                print(type(test_data.TIMESTAMP_UNIXTIME_MS_APP_EVENT_ATTRIBUTES))
...
def build_pay_load_dict(
    ...
    print(type(record["ADDRESS_USER_ATTRIBUTES"]))
    print(record)
            ...
            for source_fields, target_field in column_mapping[
                ...
                if column_mapping.get("datatype_mapping"):
                    if (
                        ...
                        == "list"
                    ) and record[str(source_fields).upper()] is not None:
                        user_attributes[target_field] = json.loads(
                            record[str(source_fields).upper()]
                        )
                    else:
                        user_attributes[target_field] = record[str(source_fields).upper()]
                else:
                    user_attributes[target_field] = record[str(source_fields).upper()]
